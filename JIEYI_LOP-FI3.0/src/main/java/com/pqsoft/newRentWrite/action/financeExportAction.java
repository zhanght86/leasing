...
public class financeExportAction extends Action {

    public VelocityContext context = new VelocityContext();

    private FiEbankUtil util = new FiEbankUtil();

    financeExportService service = new financeExportService();
    ...
    // 租金代扣页面入口(下载页面入口)
    @aPermission(name = {"资金管理", "租金管理", "财务导出", "列表"})
    ...
    public Reply cyberBank_C_Manger() {
        ...
        return new ReplyHtml(VM.html("newRentWrite/financeExport.vm", context));
    }

    // 查询按钮
    @aPermission(name = {"资金管理", "租金管理", "财务导出", "查询"})
    @aDev(code = "170051", email = "liliu@jiezhongchina.com.cn", name = "刘丽")
    ...
    // 导出下载
    @aPermission(name = {"资金管理", "租金管理", "财务导出", "下载"})
    @aDev(code = "170051", email = "liliu@jiezhongchina.com.cn", name = "刘丽")
    @aAuth(type = aAuthType.ALL)
    public Reply excelUpload() {
        ...
        if (map != null && map.get("exportType") != null && !map.get("exportType").equals("")) {
            ...
            // excel导出
            String no = new FiEbankUtil().getExportNo("租金网银-捷众excel");
            return new ReplyExcel(util.exportFinanceData(map), "rent" + DateUtil.getSysDate("yyyy-MM-dd").replaceAll("-", "") + no + ".xls");// 山重导出
        }
