$(document).ready(function(){
	...
	$('#pageTable_NotCon').datagrid({
		...
		fitColumns:true,
		toolbar:'#pageForm',
		url:'rentWriteNew!toMgAppCheckMgData.action',
		columns:[[
		...
		onExpandRow : function(index, row) {
			$('#ddv-' + row.HEAD_ID).datagrid({
                loadMsg:'',
				height:'100%',
				href : _basePath+"/rentWrite/rentWriteNew!getCheckedDetail.action?FUND_ID="+row.HEAD_ID,              	
			    onResize:function(){
...
function toCommit(obj){
	...
	$.ajax({		
		url:_basePath+"/rentWrite/rentWriteNew!doCommitHXSheet.action",
	    type:'post',
	    ...
	    success:function(json){
		 if(json.flag == true){
			...
			//页面刷新
			 window.location.href = _basePath+"/rentWrite/rentWriteNew!toMgAppCheckMg.action";
		 }else{
...
function toCommitHexiao(obj)
{
	...
	if($(tr).find("#FI_REALITY_ACCOUNT").val()==null || $(tr).find("#FI_REALITY_ACCOUNT").val()==undefined){
		...
		if(parseFloat($(tr).find("#FI_REALITY_ACCOUNT").val())<parseFloat($(tr).find("#fi_pay_money_").val())){
			...
			if(parseFloat(totalMoney)>parseFloat($(tr).find("#fi_pay_money_").val())){
				return $.messager.alert('警告','您选择金额大于本次应收核销金额，请重新选择金额！');
			}else if(parseFloat(totalMoney)<parseFloat($(tr).find("#fi_pay_money_").val())){
				return $.messager.alert('警告','您选择金额小于本次应收核销金额，请重新选择金额！');
			}else{
				data["getPoolData"] = getPoolData(tr);	
			}
		}
	...
	$.ajax({		
		url:_basePath+"/rentWrite/rentWriteNew!doCommitHXSheetHexiao.action",
	    type:'post',
	    data:'data='+encodeURI(JSON.stringify(data)),
	    dataType:'json',
	    success:function(json){
	...
	if(detailData.length<=0){
		...
		$.messager.confirm('提示','确定核销吗?',function(r) {
			if (r) {
					...
					$.ajax( {
							url : _basePath+ "/rentWrite/rentWriteNew!doCheckedFund.action",
					       type : 'post',
						   data : 'data=' + JSON.stringify(data),
					   dataType : 'json',
						success : function(json) {
							if (json.flag == true) {
								...
								window.location.href = _basePath
										+ "/rentWrite/rentWriteNew!toMgAppCheckMg.action";		
							} else {
								$.messager.alert("提示",
										"核销失败！！");
							}
