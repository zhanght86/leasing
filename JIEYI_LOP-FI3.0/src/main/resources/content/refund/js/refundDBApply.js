...
function saveRefund(){
   ...
   if(PAY_TIME==''){
	   ...
	   if(confirm("确定创建此退款单吗？")){
	       jQuery.ajax({
				url: "RefundDBApply!refundApply.action",
				data: "RE_TYPE=1&SUP_ID="+SUP_ID+"&REFUND_MONEY="+REFUND_MONEY+"&PROJECT_COUNT="+PROJECT_COUNT+"&POOL_ID_ITEMS="+POOL_ID_ITEMS+"&PAY_TIME="+PAY_TIME,			
				dataType:"json",
...
$(function(){
    $('#pageTable').datagrid({
        ...
        onExpandRow: function(index,row){
            $('#ddv-'+index).datagrid({
                url:'RefundDBApply!getRefundDropList.action?SUP_ID='+row.SUP_ID+'&STATUS=1&POOL_TYPE=1',
                singleSelect: false,
                selectOnCheck: true,
                checkOnSelect: true,
                rownumbers:true,
