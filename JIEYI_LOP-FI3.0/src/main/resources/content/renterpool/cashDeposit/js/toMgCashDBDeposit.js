$(document).ready(function(){
        	$('#pageTable').datagrid({
        		url:_basePath+"/renterpool/CashDeposit!toMgCashDBDepositData.action",
        		...
        		toolbar:'#pageForm',
        		 ...
        		 columns:[[
        		         ...
        		         {field:'BASE_MONEY',title:'DB保证金总额',width:120}, 
        		         {field:'CANUSE_MONEY',title:'DB保证金余款额度',width:120}
        		 ]]
        	});
        });
...
function returnMoney(row) {
	...
	if(PAY_STATUS == 0 || PAY_STATUS == 4 || PAY_STATUS == 7 || PAY_STATUS == 8){
		$.ajax({
			url:_basePath+"/renterpool/CashDeposit!getCust_Bank_Info.action?SUP_ID="+SUP_ID+"&SUP_NAME="+encodeURI(row.SUP_NAME)+"&TYPE=1&PAYLIST_CODE="+PAYLIST_CODE,
			type:"post",
			dataType:"json",
			success:function (json){
				if(json.flag){
					...
					$('#returnMoney').dialog('open').dialog('setTitle',PAYLIST_CODE+'DB保证金退款单');
				}else{
					alert(PAYLIST_CODE+json.msg);
				}
			}
...
function saveRefund() {
	...
	if (RE_DATE == '') {
		...
		jQuery.ajax({
			url : _basePath + "/renterpool/CashDeposit!refundDBBackSave.action",
			type : "post",
			data : "POOL_ID=" + POOL_ID_D 
					+ "&RE_MONEY="+ RE_MONEY 
					+ "&RE_DATE=" + RE_DATE
					+ "&CUST_ID=" + CUST_ID 
					...
					+ "&RE_PAYEE_UNIT=" + encodeURI(RE_PAYEE_UNIT)
...
function changeBackMoney(row) {
	...
	if(PAY_STATUS == 0 || PAY_STATUS == 4 || PAY_STATUS == 7 || PAY_STATUS == 8){
		$.ajax({
			url:_basePath+"/renterpool/CashDeposit!getSupp_Change_Info.action?PAYLIST_CODE="+PAYLIST_CODE,
			type:"post",
			dataType:"json",
			success:function (json){
				if(json.flag){
					...
					$('#changeBackMoney').dialog('open').dialog('setTitle',PAYLIST_CODE+'供应商保证金转来款');
				}else{
					alert(PAYLIST_CODE+json.msg);
				}
			}
...
function changeMoneyCome(){
	...
	if(confirm("确定转来款吗？")){
		...
		jQuery.ajax({
			url : _basePath + "/renterpool/CashDeposit!changeDBMoneyCome.action",
			type : "post",
