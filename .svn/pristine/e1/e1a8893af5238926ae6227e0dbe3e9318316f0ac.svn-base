...
function save() {
	...
	var CR_TRADE_TYPE = $("#CR_TRADE_TYPE").val();
    if (CR_TRADE_TYPE == null || CR_TRADE_TYPE == "" || CR_TRADE_TYPE == "undefined") {
        alert("请选择行业类型！");
        return;
    }
...
$(function() {
	...
	var crTradeType = $("#CR_TRADE_TYPE").val() ;
	var renterType = $("#renter_type").val() ;
	editEvaluateType(renterType,crTradeType);
});
...
function editEvaluateType(renterType,crTradeType){
	$("#EVALUATE_TYPE").empty();
	...
	jQuery.ajax({
		...
		data:{
			MAIN_TYPE:renterType,
			TRADE_TYPE:crTradeType
		} ,
		dataType:"json",
...
function addEvalu(val) {
	...
	var id = new Date().getTime();
	$("#norm")
			.append("<div class='kk'><font style='font-size:13px;font-weight:bold;'> &nbsp;定量打分项:"
