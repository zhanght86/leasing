...
function addEquipment() {
    ...
    if (param != 'view') {
		...
		$('#equipment_SUP_ID').val($('#supId').val());
		url = _basePath+'/base/suppliersInfo/AssetsDossier!doSaveEquipment.action';
    }
}
...
function delEquipment() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : _basePath+"/base/suppliersInfo/AssetsDossier!doDeleteEquipment.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function openUpdateEquipment() {
	...
	$('#UPDATE_BUY_TIME').datebox("setValue",row.BUY_TIME);
...
function updateEquipment() {
	$('#equipmentUpdate').form('submit', {
		url : _basePath+'/base/suppliersInfo/AssetsDossier!doUpdateEquipment.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
