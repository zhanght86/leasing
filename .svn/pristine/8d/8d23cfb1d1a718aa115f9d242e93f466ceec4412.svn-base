$(function(){
	$("#fiForRedTable").datagrid({
		...
		fit:true,
		fitColumns:true,
		url:_basePath+'/fiForRed/FiForRed!doShowMgFiForRedAppCW.action?PROJECT_ID='+$("#PROJECT_ID").val(),
...
function seFiForRedTable(){
	$("#fiForRedTable").datagrid('load',{
		"param":getFromData("#fiForRedForm")
		});
}
...
function FiForRedApp(TYPE){
	...
	var sta=false;
	for(var i=0; i<rows.length; i++){
		if(rows[i].PAY_STATUS!='0'&&rows[i].PAY_STATUS!='3'&&rows[i].PAY_STATUS!='8'&&rows[i].PAY_STATUS!='22'){
			sta=true;
			break;
		}
	}
