...
function updatePartnerCust(){
	...
	if (row){
		if(row.WOR_ID != undefined || row.WOR_ID != ""){
			...
			jQuery.get(_basePath+'/customers/CustMainRelation!getPartners.action?PAR_ID='+row.PAR_ID+"&date="+new Date(),function(html){
...
//查看合作伙伴 Add By YangJ 2014年5月23日 13:56:25 新增查看功能
function lookPartnerCust(){
	var row = $("#partnerDetail").datagrid('getSelected');
	if (row){
		if(row.WOR_ID != undefined || row.WOR_ID != ""){
			$("#partnerCustlook_").dialog('open');
			jQuery.get(_basePath+'/customers/CustMainRelation!getPartners.action?PAR_ID='+row.PAR_ID+"&date="+new Date(),function(html){
				$("#openPartnerlook").html("<script> $('#openPartnerlook input').attr('disabled',true); $('#openPartnerlook textarea').attr('disabled',true);$('#openPartnerlook select').attr('disabled',true);  </script>"+html);// $('#dialogButtons_').hide()
			});
		}else {
			alert("请选择一条客户合作伙伴数据");
		}
