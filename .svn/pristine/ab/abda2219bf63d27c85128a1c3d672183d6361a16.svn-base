...
function addEquipment() {
    ...
    if (param != 'view') {
		...
		url = _basePath+'/base/sp/AssetsDossier!doSaveEquipment.action';
    }
}
...
function delEquipment() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : _basePath+"/base/sp/AssetsDossier!doDeleteEquipment.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateEquipment() {
	$('#equipmentUpdate').form('submit', {
		url : _basePath+'/base/sp/AssetsDossier!doUpdateEquipment.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
