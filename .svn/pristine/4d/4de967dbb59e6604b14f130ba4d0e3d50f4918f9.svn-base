...
function getPageData(options){
	...
	jQuery.ajax({ async:false, type:'get', dataType:'html'
		...
		,global : false
		,success: function (html){ template = html;  }
	});
...
function saveScheme(){
	...
	jQuery.ajax({
		...
		,success: function (o){
			if(o.flag){
				$(T__).text('数据已保存');
				/*if(confirm("客户资料已保存，是否再来一单！\n否则将先跳转到申请列表，您也可以在业务查询中操作")){
					copyProject(o.data.PROJECT_ID );
				}else{
 ...
 * FIXME 是1.17 ? 2015-8-24
 */
function calcGZS(car_price, car_price_zd){
	...
	if( car_price < car_price_zd * 0.93 ){
		$('[name=GZS_MONEY]').val( Number(car_price_zd * 0.93 / 1.17 ).toFixed(2) );
	}else{
		$('[name=GZS_MONEY]').val( Number(car_price / 1.17).toFixed(2) );
	}
}
 ...
 * @returns {Boolean}
 */
function copyProject(pro_id){
	if( !confirm('确定要复制此项目吗？')){
		return false;
	}
	$.ajax({
		url : _basePath + '/weixin/Business!copyProject.action',
		data:{ID: pro_id },
		type:'post',
		dataType:'json'
	}).done( function (o){
		if(o.ok){
			alert('复制成功，请到业务申请列表查看');
		}else{
			alert('复制失败'+ o.error);
		}
	}).fail(function (){
