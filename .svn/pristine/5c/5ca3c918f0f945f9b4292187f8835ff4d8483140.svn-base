...
function btnSaveA(){
		...
		if(result){
			$("#payment tr").each(function(){
        		...
        		LAST_PRICE:t.find("input[name=LAST_PRICE]").val(),
        		PROJECT_ID:$("#PROJECT_ID").val()
				};
        	});
			MAINPAYMENT[0]={PAY_ID:$("#PAY_ID").val(),PROJECT_ID:$("#PROJECT_ID").val(),LOAN_AMOUNT:$("#LOAN_AMOUNT").val(),holiday_pay_type:$("#holiday_pay_type").val(),PAY_TYPE:$("#PAY_TYPE").val()};
			$("#btnSaveA").attr("disabled","true");
			jQuery.ajax({
				url:"RePaymentDetail!updatePayMentDetail.action",	
				...
				if(json.flag==true){
					alert("保存成功!");
					window.location.href=_basePath+"/rePayment/RePayment!toqueryPayDetail.action?jump_type=1&PROJECT_ID="+$("#PROJECT_ID").val();
				}else{
