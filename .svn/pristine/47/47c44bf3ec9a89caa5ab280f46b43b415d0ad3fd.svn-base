...
function compure(value,rowData){
	...
	return "<a href='javascript:void(0)' onclick=showDetail('"+rowData.ID+"','"+rowData.PROJECT_ID+"','"+rowData.PAYLIST_CODE+"')>查看</a>　"+approval;
//	alert(value);
//	alert(rowData.ID);
//	return "操作"
...
function start_jbpm(PAYLIST_CODE){
	        ...
	        dataType: "json",
			async:false,
	        url: _basePath + "/pay/PayTask!settleJBPM.action",
...
function showDetailZCJQ(ID,PAYLIST_CODE){
	top.addTab(PAYLIST_CODE+"结清明细",_basePath+"/pay/PayTask!toMgshowDetailZC.action?PAY_ID="+ID);
}
