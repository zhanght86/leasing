...
function addDebt() {
	...
	url = _basePath+'/base/suppliersInfo/CreditDossier!doSaveDebt.action';
}
...
function updateDebt() {
	$('#debtUpdate').form('submit', {
		url : _basePath+'/base/suppliersInfo/CreditDossier!doUpdateDebt.action',
...
function delDebt() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : _basePath+"/base/suppliersInfo/CreditDossier!doDeleteDebt.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
