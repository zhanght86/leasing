...
function se(){
	...
	var url_=_basePath+"/insure/Insure!renewalRemindListing.action";
	var jsonData = $("#queryForm").serialize();
...
function exportEXCEL(){
	...
	var jsonData = $("#queryForm").serialize();
	var jsonData = $("#queryForm").serialize()+"&PRO_CODES="+PRO_CODES.toString();
	//flag_标识在导出的时候同时进行状态修改
	var url_ = _basePath+"/insure/Insure!insureExportExcel.action?"+jsonData+"&status_=续保&OPERATION_TYPE=yes&status_s=待录保单信息&status_e=续保&flag_=true";
	...
	/*jQuery.ajax({
	   ...
	   async:false,
	   url: _basePath+"/insure/Insure!updateStatus.action",
	   data: jsonData+"&status_s=待录保单信息&status_e=续保",
	   success: function(msg){
	   }
	});*/
