$(function(){
	...
	$("#RECORD_NAME").change(function(){
		...
		$(".delStatus").click(function(){
			...
			var DESCRIBE=$(this).attr("DESCRIBE");
			jQuery.ajax({
				url: "DataDictionary!delDataType.action",
				data: "DATA_ID="+DATA_ID+"&STATUS="+STATUS,
				dataType:"json",
				success: function(date){
					if(date.flag==true){
...
function addType(){
	window.location.href=_basePath+"/sysDataDictionary/DataDictionary!toDataType.action";
};

function editType(row)
{	
	 if (row){
		 top.addTab(row.TYPE+"系统设置修改",_basePath+'/sysDataDictionary/DataDictionary!toDataTypeInfo.action?TYPE='+row.TYPE+'&DESCRIBE='+row.DESCRIBE);
	 }
}
