...
function addHouse() {
    ...
    if (param != 'view') {
		...
		url = _basePath+'/base/sp/AssetsDossier!doSaveHouse.action';
	}
}
function saveHouse() {
...
function delHouse() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : _basePath+"/base/sp/AssetsDossier!doDeleteHouse.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateHouse() {
	$('#houseUpdate').form('submit', {
		url : _basePath+'/base/sp/AssetsDossier!doUpdateHouse.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
