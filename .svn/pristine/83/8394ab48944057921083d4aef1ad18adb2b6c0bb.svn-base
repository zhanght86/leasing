...
import com.pqsoft.newRentWrite.service.superTableService;
...
public class superTableAction  extends Action {
    ...
    superTableService service = new superTableService();
    ...
    public Reply rentAccountMonitorPage() {
        ...
        List<Object> CLIENT_SOURCE_LIST = (List) new DataDictionaryMemcached().get("客户来源");
        context.put("CLIENT_SOURCE_LIST", CLIENT_SOURCE_LIST);
        return new ReplyHtml(VM.html("newRentWrite/superTable.vm", context));
    ...
    public Reply excelUpload() {
        ...
        if (map != null && map.get("exportType") != null && !map.get("exportType").equals("")) {
            ...
            return new ReplyExcel(util.exportSuperTable(map), "rent" + DateUtil.getSysDate("yyyy-MM-dd").replaceAll("-", "") + no + ".xls");// 山重导出
        }
