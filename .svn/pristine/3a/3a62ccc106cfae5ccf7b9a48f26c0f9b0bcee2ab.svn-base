$(document).ready(function(){
	...
	$('#pageTable_cyber_Con').datagrid({
		...
		fit:true,
		toolbar:'#pageForm',
		url:'rentWriteNew!toSupperFundMgData.action',
		...
		onExpandRow : function(index, row) {
			jQuery.ajax({
				url:_basePath+"/rentWrite/rentWriteNew!getSupperFundData.action?HEAD_ID="+row.HEAD_ID,  
				type:'post',
				dataType:'json',
...
//	var FI_REALITY_BANK=$("select[name='bankFlag']").find("option:selected").text();
	...
	var url=_basePath+"/rentWrite/rentWriteNew!toSupper_Upload.action?uploadType1="+uploadType+"&bankFlag="+bankFlag+"&FI_REALITY_BANK="+FI_REALITY_BANK+"&HEAD_ID="+HEAD_ID+"&FI_SUPPLIERS_ID="+FI_SUPPLIERS_ID+"&FI_PAY_DATE1="+FI_PAY_DATE1+"&FI_PAY_DATE2="+FI_PAY_DATE2+"&UPLOADTYPE="+UPLOADTYPE2+"&selectDateHidden="+JSON.stringify(selectData);
...
//		return true;
	...
	var url = "rentWriteNew!uploadExcel1.action";
	...
	else
	{
		$.messager.confirm("提示","您确认要重置选中的数据？",function(flag){
			if(flag){
				...
				url = _basePath+"/rentWrite/rentWriteNew!destroySuppUp.action";
		...
		$.messager.confirm('提示','确定要驳回吗?', function(r) {
			if (r) {
				...
				for(var i = 0; i<detailData.length; i++) {
					var UPLOADTYPE=detailData[i].UPLOADTYPE;
					if(UPLOADTYPE=='1'){
						alert("付款单："+detailData[i].HEAD_ID+"已导出，请先将其重置后在驳回，如已重置请点击查询在驳回！");
						return false;
					}
					var temp = {};
				...
				$.ajax({		
				    ...
				    success:function(json){
					 if(json.flag == true){
						...
						//页面刷新
							window.location.href = _basePath+"/rentWrite/rentWriteNew!toSupperFundMg.action";
					 }else{
