...
function addPayment() {
    ...
    if (param != 'view') {
		...
		$('#payment_CLIENT_ID').val($('#clientId').val());
		url = '../credit/CreditRepayment!doSavePayment.action';
	}
}
...
function delPayment() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : "../credit/CreditRepayment!doDeletePayment.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function paymentInterest() {
	$('#paymentUpdate').form('submit', {
		url : '../credit/CreditRepayment!doUpdatePayment.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
...
function down(val, row){
	if(row.FILE_NAME==null || row.FILE_NAME==""){
		...
		return "<a href='"+_basePath+"/credit/CreditRepayment!downLoadRecordFile.action?FILE_URL="+row.FILE_PATH+"&FILE_NAME="+row.FILE_NAME+"'>附件下载</a>";
	}
}
