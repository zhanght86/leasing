...
function addConfig(){
	...
	$.ajax({
		type:"post",
		url:_basePath+"/documentApp/APPDossierConfig!doAddDossierConfig.action",
...
function updateConfig(){
	...
	$.ajax({
		type:"post",
		url:_basePath+"/documentApp/APPDossierConfig!doUpdateDossierConfig.action",
...
function delConfig(ID){
	$.messager.confirm("提示","您确定要删除该条数据吗？",function(flag){
		if(flag){
			$.ajax({
				type:"post",
				url:_basePath+"/documentApp/APPDossierConfig!doDelDossierConfig.action",
				data:"ID="+ID,
				dataType:"json",
				success:function(json){
				if(json.flag){
