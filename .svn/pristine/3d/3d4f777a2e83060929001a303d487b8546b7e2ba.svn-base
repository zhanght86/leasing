...
public class InsureBXBCPayDetailAction extends Action{
	...
	private InsureBXBCPayService insureBXBCPay = new InsureBXBCPayService();
	 ...
	 */
	@Override
//	@aPermission(name = { "保险管理", "保险补差管理", "补差支付明细查询"})
...
//		return new ReplyHtml(VM.html(basePath + "insureBXBCPayDetail.vm", context));
		return null;
	}
	...
	public Reply pageData(){
		...
		Page pagedata = insureBXBCPay.getAddPageData(param);
	...
	public Reply pageDetailData(){
		...
		Page pagedata = insureBXBCPay.getDetailPageData(param);
		return new ReplyAjaxPage(pagedata);
	}
