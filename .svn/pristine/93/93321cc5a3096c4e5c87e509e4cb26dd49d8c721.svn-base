$(document).ready(function(){
	...
	$("#cyberBank_C_PageTable").datagrid({
		...
		fit:true,
		toolbar:'#pageForm',
		url:'rentWriteNew!cyberBank_C_PageAjax.action',
		...
		$.messager.confirm("提示","您确认要导出数据吗？",function(flag){
			if(flag){
				...
				if(uploadType=="select"){
					url="rentWriteNew!cyberBank_C_Upload.action?bankFlag="+$("select[name='bankFlag']").val()+"&IDS="+IDS+"&uploadType="+uploadType;
				}
				else{
...
//						FI_REALITY_BANK=$("select[name='bankFlag']").find("option:selected").text();
//					}
					url="rentWriteNew!cyberBank_C_Upload.action?ACCOUNT_NAME="+ACCOUNT_NAME+"&BEGINNING_FALSE_REASON="+BEGINNING_FALSE_REASON+"&PROJECT_CODE="+PROJECT_CODE+"&CUST_TYPE="+CUST_TYPE+"&UPLOAD_NUMBER="+UPLOAD_NUMBER+"&FEE_TYPE="+FEE_TYPE+"&SUP_NAME="+SUP_NAME+"&BANK_NAME="+BANK_NAME+"&PLAN_START_DATE="+PLAN_START_DATE+"&PLAN_END_DATE="+PLAN_END_DATE+"&SIGN_FLAG="+SIGN_FLAG+"&PRODUCT_NAME="+PRODUCT_NAME+"&bankFlag="+$("select[name='bankFlag']").val()+"&uploadType="+uploadType+"&FI_REALITY_BANK="+FI_REALITY_BANK;
				}
