...
        input.onchange = function () {//文件选择框，监听事件
			//判断是否超过最大文件数限制
			if(that.choseFilePath.length+this.files.length>that.settings.maximumFilesUpload) {
				alert("上传文件数最大不得超过"+that.settings.maximumFilesUpload);
				return;
			}
			var flag = true;
			$.each(this.files, function(i,n){
	            var fileSuf = n.name.substring(n.name.lastIndexOf(".") + 1);
	            //检查是否为允许上传的文件
	...
	            }
	            //选中后的回调
	            that.settings.onChosen(n.name, input);
	
	            if(that.checkFileIsExist(n.name)) {
	                //保存已经选择的文件路径
	                that.choseFilePath.push({ "name": input.name, "value": n.name });
	                var status = that.settings.onSameFilesHandle(n.name);
	                if (typeof status === "boolean" && !status) {
	                    that.removeFile(n.name);
						flag = false;
	                    return false;
	                }
	            } else {
	...
	                    perviewImage.beginPerview(n, that.settings.perviewImageElementId);
	                } else {
	                    var ul = perviewImage.getPerviewRegion(that.settings.perviewImageElementId);
	                    var main = perviewImage.createPreviewElement(n.name);
	...
	                    });
	                    perviewImage.beginPerview(n, div, dcmt);
	                }
	            }
	...
	            }
			...
			alert(JSON.stringify(that.choseFilePath));
			if(flag) document.getElementById("testdiv").appendChild(input);
        };
...
        this.forEach(this.choseFilePath, function (i, n) {
            if (n.value == file) {
				$("#testdiv input[name="+n.name+"]").remove();
...
        if (file) { //此处为Firefox，Chrome以及IE10的操作
...
                preview_div.appendChild(div);
				div.title = '点击图片可在新窗口打开';
//				alert(src);
				div.onclick = function() { window.open(src);}
            }
