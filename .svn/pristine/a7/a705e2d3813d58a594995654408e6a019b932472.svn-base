...
//	}
	//add JZZL-201  Start
	$("#SCALE_SOURCE").change(function(){
		var $offlineSpan = $("#offlineSpan");
		var $offlineChannel = $("#offlineChannel");
		//alert($(this).val());
		if("线下-渠道合作" == $(this).val()){
			$offlineSpan.show();
			$offlineChannel.addClass("warm");
		}else{
			$offlineSpan.hide();
			$offlineChannel.val("");
			$offlineChannel.removeClass("warm");
		}
	});
	//add JZZL-201  END
});
...
function checkCardNo(cardId){
	...
	if(id_card_type == "1" || id_card_type == "4"){
		if(isCardNo(cardId,id_card_type)){	
			...
			$.ajax({
				type:"post",
				url:_basePath + "/customers/Customers!checkBlackCust.action?ID_CARD_NO="+cardNo,
				dataType:"json",
				success:function(json){
				if(json.data){
					 $("#alertInfo").attr("color","red");    
					$("#alertInfo").text("黑名单不允许进件");
					$("#"+cardId).val("");
					$("#"+cardId).focus();
				}else{
					 $("#alertInfo").attr("color","green");    
					$("#alertInfo").text("验证通过");
						$.ajax({
							type:"post",
...
//	$("#updateCust"+CLIENT_ID).find("#updateSave").linkbutton("disable");
	...
	//del gengchangbao JZZL-241 Start
	//验证组织机构代码证\税务登记证号
...
function getCustData(save) {
	...
	$("#"+save).each(function(){
		...
		temp.offlineChannel = $(this).find($("#offlineChannel")).val();
...
//			alert("comboname:"+name +" value: "+ zdy_array[s]) ;
		...
		$("#"+save).find("input[name^='EMERGENCY_NAME']").each(function(){
			var t = {}  ;
			// 联系人ID
			var EMERGENCY_ID = $("#"+save).find("input[name='EMERGENCY_ID_"+ i +"']").val();
			t.EMERGENCY_ID = EMERGENCY_ID;
...
function appendEmergencyContact(){
	...
	var QUYUOPTION=$("#QUYUOPTION").val(); // 居住省
	var contact_type =$("#CONTACT_TYPE").val(); // 联系人类型
	var relationship =$("#RELATIONSHIP").val(); // 与申请人关系
	
	var  emergencyTitle = $("#emergencyTitle") ; // 取得TD
	var rowspan = emergencyTitle.attr("rowCounts") ; // 取得当前行数
	var nextRowSpan = parseInt(rowspan) + 5;		// 增加一个TR，增加5行
	...
	if(emergencyEndTr!= null && emergencyEndTr!= undefined){
		...
		var next = parseInt(nextRowSpan)/5 ; //取整后的联系人（TD）行数
		var removeSpan = next-1; // 上个联系人行数值
//		$("#span"+removeSpan).remove() ;
		$("#span" + removeSpan).html(""); // 把SPAN的内容设置空
...
//				 		"<input type='text' class='warm' id='EMERGENCY_RELATIONSHIP_"+next+"' title='联系人与申请人关系' style='width:135px;' name='EMERGENCY_RELATIONSHIP_"+next+"'/>"+
				 	...
				 	"<tr class='tr"+ next +"'>"+
					 	...
					 	"</td>"+
						 "<td colspan='2'>" +
						 	"<input type='text' id='EMERGENCY_ADDR_"+next+"'  style='width:415px;' name='EMERGENCY_ADDR_"+next+"'/>" +
				 		"</td>" +
				 	"</tr>" ;			
...
//	alert("next:" + next + "\n pre:" + pre);
	// 追加的行大于1的时候
	if(	pre > 1 ){
		var span = "(<a href='javascript:void(0)' onclick='removeEmergencyContact("+pre+")'>移除</a>)";
		$("#span"+ pre).html(span);
	}
}
