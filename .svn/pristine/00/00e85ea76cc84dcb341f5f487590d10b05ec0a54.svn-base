...
function approval(parm){
	...
	var url_ = _basePath+"/insure/Insure!insureAffirmApproval.action";
	jQuery.ajax({
	   type: "POST",
	   dataType:"json",
	   url: url_,
...
function se(){
	//当url上面的type_=2时候为审核
	var url_=_basePath+"/insure/Insure!insureAffirmListing.action";
	var jsonData = $("#queryForm").serialize();
...
function exportEXCEL(){
	...
	var url_ = _basePath+"/insure/Insure!insureCheckExportExcel.action?"+jsonData+"&status_=预约";
