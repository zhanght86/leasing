 ...
 */
public class DossierConfigAction extends Action {
	private String path = "dossier/";
	private final Logger log = Logger.getLogger(this.getClass());
	private DossierConfigService service = new DossierConfigService();
	...
	@aPermission(name = { "档案管理", "档案柜编码", "列表显示" })
	...
	@aDev(code = "170053", email = "jinfu@pqsoft.cn", name = "King")
//	@aPermission(name = { "档案管理", "档案柜编码", "查询" })
	...
	@aAuth(type = aAuthType.USER)
	@aDev(code = "170053", email = "jinfu@pqsoft.cn", name = "King")
	@aPermission(name = { "档案管理", "档案柜编码",  "添加" })
	public Reply doAddDossierConfig() {
		...
		if (i > 0) {
			return new ReplyAjax().addOp(new OpLog("档案管理-档案柜编码", "添加保存", _getParameters() + ""));
		} else
	...
	@aAuth(type = aAuthType.USER)
	@aDev(code = "170053", email = "jinfu@pqsoft.cn", name = "King")
	@aPermission(name = { "档案管理", "档案柜编码", "删除" })
	public Reply doDelDossierConfig(){
		log.info(_getParameters());
		int i=service.doDelDossierConfig(_getParameters());
		if(i>0){
			return new ReplyAjax().addOp(new OpLog("档案管理-档案柜编码", "删除", _getParameters()+""));
		}else{
