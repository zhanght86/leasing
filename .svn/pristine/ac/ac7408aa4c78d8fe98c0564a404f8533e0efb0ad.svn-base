...
$(function(){
		table1  = $('#table1').datagrid({
			url : 'BankSignMg!doSelectBankSignApplyPageDataCX.action',
			columns : [[
...
//				}},
				...
				{field:'AGREEMENTNO',title:'签约协议号',width:150,align:'center'},
...
//导出
function exportExcelNewFCJY(flag){
	//url
	var url = "BankSignMg!exportExcelNewFCJY.action";
...
function uploadExcel(){
	...
	var url = "BankSignMg!uploadExcel.action";
							...
							$('#fileUploadForm').form('submit',
								{
                                    onSubmit: function()
									{
										...
										if($('#uploadFile').val().indexOf('.xls') == -1 &&  (bank_type =="2" || bank_type =="6"))
										{
                                    ...
                                    url:url+"?bankFlag="+$('#bankFlag').combobox("getValue")+"&fileN="+$('#uploadFile').val(),
