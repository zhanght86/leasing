...
function addPayment() {
    ...
    if (param != 'view') {
		...
		url = _basePath+'/base/suppliersInfo/CreditDossier!doSavePayment.action';
	}
}
...
function delPayment() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : _basePath+"/base/suppliersInfo/CreditDossier!doDeletePayment.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function paymentInterest() {
	$('#paymentUpdate').form('submit', {
		url : _basePath+'/base/suppliersInfo/CreditDossier!doUpdatePayment.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
...
function upRecord() {
    ...
    jQuery.ajaxFileUpload({
	    url:_basePath+"/base/suppliersInfo/CreditDossier!uploadRecord.action",
	    secureuri:false,
...
function down(val, row){
	if(row.FILE_NAME==null || row.FILE_NAME==""){
		...
		return "<a href='"+_basePath+"/base/suppliersInfo/CreditDossier!downLoadRecordFile.action?FILE_URL="+row.FILE_PATH+"&FILE_NAME="+row.FILE_NAME+"'>附件下载</a>";
	}
}
