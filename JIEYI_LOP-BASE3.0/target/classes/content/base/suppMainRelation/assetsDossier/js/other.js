...
function addOther() {
    ...
    if (param != 'view') {
		...
		url = _basePath+'/base/suppliersInfo/AssetsDossier!doSaveOther.action';
	}
}
function saveOther() {
...
function delOther(ID) {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : _basePath+"/base/suppliersInfo/AssetsDossier!doDeleteOther.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateOther() {
	$('#otherUpdate').form('submit', {
		url : _basePath+'/base/suppliersInfo/AssetsDossier!doUpdateOther.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
