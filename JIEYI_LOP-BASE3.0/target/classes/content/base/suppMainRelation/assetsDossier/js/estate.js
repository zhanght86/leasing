...
function addEstate() {
    ...
    if (param != 'view') {
		...
		$('#estate_SUP_ID').val($('#supId').val());
		url = _basePath+'/base/suppliersInfo/AssetsDossier!doSaveEstate.action';
	}
}
function saveEstate() {
...
function delEstate(ID) {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : _basePath+"/base/suppliersInfo/AssetsDossier!doDeleteEstate.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateEstate() {
	$('#estateUpdate').form('submit', {
		url : _basePath+'/base/suppliersInfo/AssetsDossier!doUpdateEstate.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
