...
function onload(){
 jQuery("#anchoredTable").datagrid({
	 ...
	 fit:true,
	 fitColumns:true,
	 url:_basePath+"/anchored/AnchoredManager!doShowAnchored.action",
	 toolbar:"#pageForm",
	 columns:[[
		      {field:'NAME',width:35,title:'挂靠公司名称'},
		      ...
		      {field:'SUP_SHORTNAME',width:35,title:'所属供应商'},
...
function destroyType(ID){
	$.messager.confirm("删除","确定要删除该条数据吗？",function(r){
		if(r){
			$.ajax({
				type:'post',
				url:_basePath+"/anchored/AnchoredManager!doDeleteAnchored.action",
				data:"ID="+ID,
				dataType:"json",
...
function addAnchored(){
	...
	var SUP_ID=$("#addAchoredForm").find("#SUP_ID").combobox('getValue');
	var QUALIFICATION=$("#addAchoredForm").find("#QUALIFICATION").combobox('getValue');
	
	$.ajax({
		type:'post',
		url:_basePath+"/anchored/AnchoredManager!doAddAnchored.action",
		data:"NAME="+encodeURI(NAME)+"&ORGAN_CODE="+encodeURI(ORGAN_CODE)+"&ADDRESS="+encodeURI(ADDRESS)+
			 "&POST="+encodeURI(POST)+"&TELPHONE="+encodeURI(TELPHONE)+"&SUP_ID="+SUP_ID+"&QUALIFICATION="+
...
function updateAnchored(){
	...
	var SUP_ID=$("#updateAchoredForm").find("#SUP_ID").combobox('getValue');
	
	$.ajax({
		type:'post',
		url:_basePath+"/anchored/AnchoredManager!doUpdateAnchored.action",
