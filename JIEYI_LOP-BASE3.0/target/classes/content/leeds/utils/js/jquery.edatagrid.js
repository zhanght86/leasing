...
(function($) {
	var currTarget;
	$(function() {
				$(document).unbind('.edatagrid').bind('mousedown.edatagrid',
						function(e) {
							var p = $(e.target)
					...
					if (row.isNewRecord) {
						...
						if (opts.destroyUrl) {
							...
							$.post(opts.destroyUrl, {
										...
										if (pager.length
												&& !dg.datagrid('getRows').length) {
											dg.datagrid('options').pageNumber = pager
	...
	$.fn.edatagrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
				...
				autoSave : false, // auto save the editing row when click out
									// of datagrid
