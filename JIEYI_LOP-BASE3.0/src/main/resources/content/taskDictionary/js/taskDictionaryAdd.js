...
function openUpdateTaskDic(row){
	...
	var SUPPLIERS = row.SUPPLIER_ID;
	var strs = SUPPLIERS.split(",");
	for ( var i = 0; i < strs.length; i++) {
		// 设置选中  
		$('#UPDATE_SUPPLIER_ID').combobox("select", strs[i]);
	}
}
...
//执行修改操心
function updateTaskDic(){
	var TASK_ID=$("#updateTaskDicDialog").find("[name=TASK_ID]").val();
	var NODE_NAMES = $('#UPDATE_NODE_NAME').combobox('getValues');
	var SUPPLIER_IDS = $('#UPDATE_SUPPLIER_ID').combobox('getValues');
...
function setTaskDicTool(val,row){
	return "<a href='#' onclick='openUpdateTaskDic(" + JSON.stringify(row) + ")'>修改</a>  |  <a href='#' onclick='delTaskDicInfo(" + JSON.stringify(row) + ",1)'>删除</a> ";
}
//删除配置信息 
