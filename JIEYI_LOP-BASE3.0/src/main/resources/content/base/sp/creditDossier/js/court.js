...
function addCourt() {
	...
	url = _basePath+'/base/sp/CreditDossier!doSaveCourt.action';
}
...
function updateCourt() {
	$('#courtUpdate').form('submit', {
		url : _basePath+'/base/sp/CreditDossier!doUpdateCourt.action',
...
function delCourt() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : _basePath+"/base/sp/CreditDossier!doDeleteCourt.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
