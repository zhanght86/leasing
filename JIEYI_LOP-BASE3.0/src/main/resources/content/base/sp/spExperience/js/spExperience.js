...
//	alert(param) ;
	$.ajax({
		url:_basePath+"/base/sp/SpMainRelation!doInsertWoekExp.action",
...
function toUpdateExperience(){
	...
	$.ajax({
		url:_basePath+"/base/sp/SpMainRelation!doUpdateExperence.action",
...
function getUpdateExperience_(){
	...
	if (row){
		if(row.ID != undefined && row.ID != ""){
			...
			jQuery.get(_basePath+'/base/sp/SpMainRelation!getUpdateExperience.action?ID='+row.ID+"&date="+new Date(),function(html){
...
function delExperience(){
	if(confirm("确认删除？")){
		...
		if (row.ID){
			jQuery.ajax({
				url: _basePath+"/base/sp/SpMainRelation!delWoekExp.action",
				data: "ID="+row.ID,
				dataType:"json",
				success : function(data) {
							if (data.flag == true) {
