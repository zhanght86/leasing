...
function addHouse() {
    ...
    if (param != 'view') {
		...
		$('#houseAdd').form('clear');
		$('#house_CLIENT_ID').val($('#clientId').val());
		$('#house_CREDIT_ID').val($('#creditId').val());
		url = '../credit/CreditAssetsDossier!doSaveHouse.action';
	}
}
function saveHouse() {
...
function delHouse() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : "../credit/CreditAssetsDossier!doDeleteHouse.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateHouse() {
	$('#houseUpdate').form('submit', {
		url : '../credit/CreditAssetsDossier!doUpdateHouse.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
