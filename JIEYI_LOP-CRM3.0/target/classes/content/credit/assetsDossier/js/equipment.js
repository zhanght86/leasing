...
function addEquipment() {
    ...
    if (param != 'view') {
		...
		$('#equipment_CREDIT_ID').val($('#creditId').val());
		url = '../credit/CreditAssetsDossier!doSaveEquipment.action';
    }
}
...
function delEquipment() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : "../credit/CreditAssetsDossier!doDeleteEquipment.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateEquipment() {
	$('#equipmentUpdate').form('submit', {
		url : '../credit/CreditAssetsDossier!doUpdateEquipment.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
