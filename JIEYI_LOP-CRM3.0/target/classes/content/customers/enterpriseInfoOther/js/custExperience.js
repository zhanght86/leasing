...
function toExperience(div){
	...
	$("#Exp_CLIENT_ID").val($("#client_id").val());
}
...
function toSaveExperience(){
	...
	//alert(param) ;
	$.ajax({
		url:_basePath+"/customers/CustMainRelation!doInsertWoekExp.action",
...
function getUpdateExperience_(){
	...
	if (row){
		if(row.WOR_ID != undefined && row.WOR_ID != ""){
			...
			jQuery.get(_basePath+'/customers/CustMainRelation!getUpdateExperience.action?WOR_ID='+row.WOR_ID+"&date="+new Date(),function(html){
...
function toUpdateExperience(){
	...
	$.ajax({
		url:_basePath+"/customers/CustMainRelation!doUpdateExperence.action",
...
function delExperience(){
	if(confirm("确认删除？")){
		...
		if (row){
			jQuery.ajax({
				url: _basePath+"/customers/CustMainRelation!delWoekExp.action",
				data: "WOR_ID="+row.WOR_ID,
				dataType:"json",
				success : function(data) {
							if (data.flag == true) {
								 $.messager.show({
...
//开始日期和结束日期比较
function startComparisonEnd(){
	var startDate =$("input[name='START_DATE']").val();
	var endDate =$("input[name='END_DATE']").val();
	var nowDate =new Date();
	var startDateNew = new Date(startDate.replace(/-/g,"/"));
	var endDateNew = new Date(endDate.replace(/-/g,"/"));
	if((startDate!=null && startDate!='') && (endDate!=null && endDate!='')){
		if(startDateNew >= endDateNew){
			alert("开始日期不能大于结束日期!");
			return false;
		}
		if(endDateNew > nowDate){
			alert("结束日期不能大于当前日期!");
			return false;
		}
		return true;
