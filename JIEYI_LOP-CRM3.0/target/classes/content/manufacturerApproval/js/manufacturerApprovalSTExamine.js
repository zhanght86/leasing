...
function exportWord(PROJECT_ID,KH_TYPE){
	//发送导出请求
	//FILETYPE控制个人还是法人模板
	window.location.href=_basePath+"/manufacturerApproval/ManufacturerApproval!exportSTGFWord.action?PROJECT_ID=" + PROJECT_ID + "&FILETYPE=" + KH_TYPE;
}
...
function save(){
	...
	var DESCRIPTION = $("#DESCRIPTION").attr("value");//起租比例说明
	...
	$.messager.confirm("添加","确定要保存该审批意见吗?",function(r){
		if(r){
			jQuery.ajax({
				...
				data : { "PROJECT_ID": PROJECT_ID,"PROJ_ID": PROJ_ID,"MANUFACTURER" : MANUFACTURER,"ADVICE" : ADVICE,"DESCRIPTION" : DESCRIPTION,"DLD" : DLD},
				dataType:'json',
				success:function(date){
					alert("保存成功");
					//关闭页面
					top.closeTab("山推股份厂商项目审批");
