...
public class FinancialStatisticsService {
	private final String xmlPath = "financialStatistics.";
	...
	* @author YuanYe
	...
	public void testCalculate(Map<String, Object> map, VelocityContext context,
		...
		if(null!=financeManager.get("IN_DATE1")){
			map.put("IN_DATE", financeManager.get("IN_DATE1"));
		}else{
			map.put("IN_DATE", "");
		}
		if(null!=financeManager.get("IN_DATE2")){
			map.put("IN_DATE1", financeManager.get("IN_DATE2"));
		}else{
			map.put("IN_DATE1", "");
		}
		if(null!=financeManager.get("IN_DATE3")){
