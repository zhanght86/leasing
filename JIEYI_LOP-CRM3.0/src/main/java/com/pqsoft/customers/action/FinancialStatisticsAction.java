...
public class FinancialStatisticsAction extends Action {
	...
	private final String pagePath = "customers/financialStatistics/";
...
//	@aPermission(name = { "客户管理", "客户资料管理", "法人财务报表主页面" })
	public Reply execute() {
		Map<String, Object> param = _getParameters();
		VelocityContext context = new VelocityContext();
		context.put("param", param);
		return new ReplyHtml(VM
				.html(pagePath + "toEditFinanceMain.vm", context));
	}

	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "法人财务报表编辑" })
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "法人财务报表查看" })
...
//		System.out.println("----------------map=" + map);
		...
		if (map.get("SORT") != null && !"".equals(map.get("SORT"))) {
			if (map.get("SORT").equals("1")) {
				...
				logger.debug("资产负债表");
					return new ReplyHtml(VM.html(pagePath
							+ "toShowFinance_debt.vm", context));
			} else if (map.get("SORT").equals("2")) {
				service.queryProfitByID(map, context);
				logger.debug("损益表");
					return new ReplyHtml(VM.html(pagePath
							+ "toShowfinnace_Profit.vm", context));
	
			} else if (map.get("SORT").equals("3")) {
				logger.debug("现金流量表");
				service.queryCashById(map, context);
					return new ReplyHtml(VM.html(pagePath
							+ "toShowfinnace_Cash.vm", context));
			} else if (map.get("SORT").equals("4")) {
				logger.debug("财务指标分析");
				service.queryFinanceAnalyzeById(map, context);
					return new ReplyHtml(VM.html(pagePath
							+ "toShowfinnace_BankStatistics.vm", context));
			}
		} else {// 默认的查询当前用户的所有任务
			logger.debug("默认的查询当前用户的所有任务");
		}
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "资产负债[保存]" })
	public Reply doInsertDebt() {
		...
		return new ReplyAjax(flag).addOp(new OpLog("客户资料管理", "添加", "资产负债[保存]"));
	}

	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "", email = "yuanye@pqsoft.cn", name = "陈原野")
//	@aPermission(name = { "客户管理", "客户资料管理", "资产负债[修改]" })
	public Reply doEditDebt() {
		...
		return new ReplyAjax(flag).addOp(new OpLog("客户资料管理", "修改", "资产负债[修改]"));
	}

	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "损益表[保存]" })
	public Reply doInsertProfitDistri() {
		...
		if (returnValue > 0) {
			result = "保存成功";
			return new ReplyAjax(result).addOp(new OpLog("客户资料管理", "添加",
					"损益表[保存]"));
		} else {
			result = "保存失败";
			return new ReplyAjax(result).addOp(new OpLog("客户资料管理", "添加",
...
//	@aPermission(name = { "客户管理", "客户资料管理", "损益表[保存]" })
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "现金流量[保存]" })
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "现金流量[修改]" })
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "财务指标测算" })
	public Reply testFinancialIndex() {
		Map<String, Object> map = _getParameters();
		// 盈利能力
		List<Map<String, Object>> profitability = (List<Map<String, Object>>) service
				.profitability(map);
		map.put("profitability", profitability);

		// 偿债能力
		List<Map<String, Object>> debtAbility = (List<Map<String, Object>>) service
				.debtAbility(map);
		map.put("debtAbility", debtAbility);

		// EBITDA
		List<Map<String, Object>> EBITDA = (List<Map<String, Object>>) service
				.statisticsEBITDA(map);
		map.put("EBITDA", EBITDA);

		// 经营现金流分析
		List<Map<String, Object>> jyxjFlowAnalyze = (List<Map<String, Object>>) service
				.jyxjFlowAnalyze(map);
		map.put("jyxjFlowAnalyze", jyxjFlowAnalyze);

		// 销售收入增长率
		List<Map<String, Object>> salesGrowthRate = (List<Map<String, Object>>) service
				.salesGrowthRate(map);
		map.put("salesGrowthRate", salesGrowthRate);

		// 总资产增长率
		List<Map<String, Object>> AssetsGrowthRate = (List<Map<String, Object>>) service
				.AssetsGrowthRate(map);
		map.put("AssetsGrowthRate", AssetsGrowthRate);

		// 运营能力
		List<Map<String, Object>> servicePower = (List<Map<String, Object>>) service
				.servicePower(map);
		map.put("servicePower", servicePower);

		return new ReplyJson(JSONObject.fromObject(map)).addOp(new OpLog(
				"客户资料管理", "添加", "财务指标测算"));
	}

	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "财务指标[保存]" })
...
//	@aPermission(name = { "客户管理", "客户资料管理", "财务指标[保存]" })
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170039", email = "yangxue@pqsoft.cn", name = "杨雪")
//	@aPermission(name = { "客户管理", "客户资料管理", "财务指标[查看]" })
	...
	@aDev(code = "", email = "yuanye806@sina.con", name = "陈原野")
	@aPermission(name = { "业务管理", "客户管理", "修改","财务报表"})
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "", email = "yuanye806@sina.con", name = "陈原野")
//	@aPermission(name = { "客户管理", "客户资料管理", "修改","财务报表"})
	...
	@aAuth(type = aAuthType.USER)
	@aDev(code = "", email = "yuanye806@sina.con", name = "陈原野")
	@aPermission(name = { "业务管理", "客户管理", "查看","财务报表"})
	public Reply manageFinancialView() {
		...
		context.put("financialList", service.manageFinancial(map));
		return new ReplyHtml(VM
				.html(pagePath + "manageFinanceView.vm", context));
