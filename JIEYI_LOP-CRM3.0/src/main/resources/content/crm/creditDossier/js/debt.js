...
function addDebt() {
	...
	$('#debtAdd').form('clear');
	$("input[name='DEBT_NAME']").val($("#debt_CLIENT_NAME").val());
	$('#debt_CLIENT_ID').val($('#clientId').val());
	url = '../crm/CreditDossier!doSaveDebt.action';
}
...
function updateDebt() {
	$('#debtUpdate').form('submit', {
		url : '../crm/CreditDossier!doUpdateDebt.action',
...
function delDebt() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : "../crm/CreditDossier!doDeleteDebt.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
