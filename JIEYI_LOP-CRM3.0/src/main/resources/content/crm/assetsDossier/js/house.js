...
function addHouse() {
    ...
    if (param != 'view') {
		...
		$('#houseAdd').form('clear');
		$("input[name='OWNER_NAME']").val($("#house_CLIENT_NAME").val());
		$('#house_CLIENT_ID').val($('#clientId').val());
		url = '../crm/AssetsDossier!doSaveHouse.action';
	}
}
function saveHouse() {
...
function delHouse() {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : "../crm/AssetsDossier!doDeleteHouse.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateHouse() {
	$('#houseUpdate').form('submit', {
		url : '../crm/AssetsDossier!doUpdateHouse.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
