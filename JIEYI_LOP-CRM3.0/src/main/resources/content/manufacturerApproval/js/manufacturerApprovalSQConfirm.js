...
//TODO hxl
function exportConfirmExcel(PROJECT_ID){
	var CAR_SALE_DATE = $("#CAR_SALE_DATE").datebox("getValue");//车辆已售日期
	var ADVICE = $("#ADVICE").attr("value");//意见
	//发送导出请求
	window.location.href=_basePath+"/manufacturerApproval/ManufacturerApproval!exportConfirmExcel.action?PROJECT_ID=" + PROJECT_ID + "&CAR_SALE_DATE=" + CAR_SALE_DATE + "&ADVICE=" + ADVICE;
}
	...
	$.messager.confirm("添加","确定要保存该审批意见吗?",function(r){
		if(r){
			jQuery.ajax({
				...
				data : { "PROJECT_ID": PROJECT_ID,"PROJ_ID": PROJ_ID,"MANUFACTURER" : MANUFACTURER,"ADVICE" : ADVICE,"CAR_SALE_DATE" : CAR_SALE_DATE,"DLD" : DLD},
				dataType:'json',
				success:function(date){
					$.messager.confirm("提示","保存成功",function(r){
						//关闭页面
						top.closeTab("陕重汽厂商项目审批确认函");
