 ...
 * 查看 获取投资情况数据
 * @author 杨杰
 * date 2014年5月26日 12:13:16
 * @return
 */
function getInversInfolook(){
	var row = $("#inversInfoDetail").datagrid('getSelected');
	if (row){
		if(row.WOR_ID != undefined || row.WOR_ID != ""){
			$("#updateinversInfolook_").dialog('open');
			jQuery.get(_basePath+'/customers/CustMainRelation!getInvest.action?INFO_ID='+row.INFO_ID+"&date="+new Date(),function(html){
				$("#openDialoglook").html( "<script> $('#openDialoglook input').attr('disabled',true); $('#openDialoglook textarea').attr('disabled',true);$('#openDialoglook select').attr('disabled',true);  </script>" + html);
			});
		}else {
			alert("请选择一条客户投资情况数据");
		}
