...
function addEstate() {
    ...
    if (param != 'view') {
		...
		$('#CREDIT_ID').val($('#creditId').val());
		url = '../credit/CreditAssetsDossier!doSaveEstate.action';
	}
}
function saveEstate() {
...
function delEstate(ID) {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : "../credit/CreditAssetsDossier!doDeleteEstate.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateEstate() {
	$('#estateUpdate').form('submit', {
		url : '../credit/CreditAssetsDossier!doUpdateEstate.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
