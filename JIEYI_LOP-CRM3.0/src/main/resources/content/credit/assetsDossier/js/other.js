...
function addOther() {
    ...
    if (param != 'view') {
		...
		$('#otherAdd').form('clear');
		$('#other_CLIENT_ID').val($('#clientId').val());
		$('#other_CREDIT_ID').val($('#creditId').val());
		url = '../credit/CreditAssetsDossier!doSaveOther.action';
	}
}
function saveOther() {
...
function delOther(ID) {
	...
	if (confirm("确定要删除此记录？")) {
		jQuery.ajax( {
			url : "../credit/CreditAssetsDossier!doDeleteOther.action",
			data : "ID=" + row.ID,
			dataType : "json",
			success : function(date) {
				if (date.flag == true) {
...
function updateOther() {
	$('#otherUpdate').form('submit', {
		url : '../credit/CreditAssetsDossier!doUpdateOther.action',
		success : function(date) {
			date = $.parseJSON(date);
			if (date.flag) {
