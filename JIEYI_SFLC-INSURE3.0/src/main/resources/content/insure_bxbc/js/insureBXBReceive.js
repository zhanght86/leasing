...
function showAddPage(){
	top.addTab(" 新增 - (保险补差费用)收取收款", _basePath + "/insurebxbc/InsureBXBCReceive!toShowAddApply.action");
}
	...
	$.messager.confirm("删除","确定要删除该申请吗?",function(r){
		if(r){
			$.ajax({
				type: "POST",
			    dataType:"json",
			    url: _basePath +'/insurebxbc/InsureBXBCReceive!deleteBCSQApply.action',
	...
	$.messager.confirm("提交","确定要提交该申请吗?",function(r){
		if(r){
			$.ajax({
				type: "POST",
			    dataType:"json",
			    url: _basePath +'/insurebxbc/InsureBXBCReceive!commitBCSQApply.action',
			    data: "ID=" + datagridList[0].ID,//样式控制只能选择单条
			    success: function(flag){
