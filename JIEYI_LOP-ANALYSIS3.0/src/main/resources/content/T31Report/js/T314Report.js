...
function savePage() {
	var DATA_TIME_YEAR = $("#DATA_TIME_YEAR").val();
	  ...
	  if(DATA_TIME_YEAR != null && DATA_TIME_YEAR != "" ){
		  $("#DATA_TIME").val(DATA_TIME_YEAR+"-01");
	  }else{
...
function openUpdatePage() {
	...
	$('#UP_COLUMN11').val(row.COLUMN11);
	$('#UP_COLUMN12').val(row.COLUMN12);
...
function updatePage() {
	var UP_DATA_TIME_YEAR = $("#UP_DATA_TIME_YEAR").val();
	  ...
	  if(UP_DATA_TIME_YEAR != null && UP_DATA_TIME_YEAR != ""){
		  $("#UP_DATA_TIME").val(UP_DATA_TIME_YEAR+"-01");
	  }else{
		  $.messager.alert("提示","请选择数据时点!");
		  return;
	  }
