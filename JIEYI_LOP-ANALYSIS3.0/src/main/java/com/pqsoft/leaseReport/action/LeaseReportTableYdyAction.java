...
public class LeaseReportTableYdyAction extends Action {
	
	private final String _PATH = "leaseYdyReprotTable/";
	// **实现类服务
	LeaseReportTableService service = new LeaseReportTableService();

	//页面自定义显示列的下拉列表所需，同时也是sql所需
	private String[] titlesName = new String[] { "状态", "客户名称", "合同编号", "支付表编号", "抵押期限", "到期提醒", "已抵押原因" };
	...
	@aPermission(name = { "报表管理", "租赁物抵押报表", "租赁物已抵押统计" })
	...
	public Reply execute() {
		...
		param.put("DATA_TYPE1", "YDY");
		context.put("chartReport", utilChart.to_chartPie(param, "报表-租赁物抵押-分析-统计类型", "已抵押统计", "leaseReport.showGroupLeaseType", "1"));
		return new ReplyHtml(VM.html(_PATH+"graphTJLeaseChart.vm", context));
	}
	
	@SuppressWarnings("unchecked")
	@aAuth(type = aAuth.aAuthType.USER)
	@aPermission(name = { "报表管理", "租赁物抵押报表", "租赁物已抵押分析" })
	@aDev(code = "fuyulong", email = "fuyulong47@foxmail.com", name = "付玉龙")
	public Reply ydyFX() {
		Map<String,Object> param = _getParameters();
		VelocityContext context = new VelocityContext();
		...
		json.put("DATA_TYPE1", "YDY");
		context.put("chartReport", new UtilChart().to_chartLine(json, "报表-租赁物抵押-分析-统计类型", "已抵押分析", "leaseReport.REPORT_X_MONTH,leaseReport.LEASE_REPORT_TYPE,leaseReport.showGroupLeaseFX","1"));
	...
	@aDev(code = "fuyulong", email = "fuyulong47@foxmail.com", name = "付玉龙")
	@aPermission(name = { "数据统计", "租赁物抵押报表", "已抵押统计" })
	@SuppressWarnings("unchecked")
	public Reply toChartLease() {
		...
		context.put("REPORT_TYPE_list", REPORT_TYPE_list);
		/********************************控制自定义显示列********************/
		context.put("columnInit", utilChart.parseColumnInit("已抵押统计", titlesName, sqlsName));
		/********************************************************************/
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "fuyulong", email = "fuyulong47@foxmail.com", name = "付玉龙")
	@aPermission(name = { "数据统计", "租赁物抵押报表", "已抵押分析" })
	@SuppressWarnings("unchecked")
	...
	public Reply doChartLease() {
		...
		if ("2".equals(json.get("REPORT_QZTX"))) {
			...
			json.put("DATA_TYPE1", "YDY");
			chart = new UtilChart().to_chartLine(json, "报表-租赁物抵押-分析-统计类型", "已抵押分析", "leaseReport.REPORT_X_MONTH,leaseReport.LEASE_REPORT_TYPE,leaseReport.showGroupLeaseFX", "1");

		} else {
			// 饼状图
			JSONObject param1 = JSONObject.fromObject(_getParameters().get("param"));
			param1.put("DATA_TYPE1", "YDY");
			chart = utilChart.to_chartPie(param1, "报表-租赁物抵押-分析-统计类型", "已抵押统计", "leaseReport.showGroupLeaseType", "1");
		}
	...
	public Reply doChartLeaseData() {
		...
		if ("2".equals(json.get("REPORT_QZTX"))) {
			...
			json.put("DATA_TYPE1", "YDY");
			chart = new UtilChart().to_chartLine(json, "报表-租赁物抵押-分析-统计类型", "已抵押分析", "leaseReport.REPORT_X_MONTH,leaseReport.LEASE_REPORT_TYPE,leaseReport.showGroupLeaseFX", "2");
			
		} else {
			// 饼状图
			JSONObject param1 = JSONObject.fromObject(_getParameters().get("param"));
			param1.put("DATA_TYPE1", "YDY");
			chart = utilChart.to_chartPie(param1, "报表-租赁物抵押-分析-统计类型", "已抵押统计", "leaseReport.showGroupLeaseType", "2");
		}
		return new ReplyAjax(true, chart, null);
	}
	...
	public ReplyAjax exportExcel(){
		...
		m.put("DATA_TYPE1", "YDY");
		new POIExcelUtil().expExcel_2007(m,"已抵押统计"+"_"+DateUtil.getSysDate()+"_"+Math.abs(new Random(100).nextInt())+".xlsx", "已抵押统计", titleName,sql_columns, service.leaseList(m));
	...
	public Reply doTableShow() {
		...
		if(param.get("COLUMN_NAMES") != null){
			utilChart.delReportColumnByReportAndUser("已抵押统计", Security.getUser().getCode());;
			utilChart.insertReportColumnByReportAndUser("已抵押统计", Security.getUser().getCode(),param.get("COLUMN_NAMES"));
		}
