package com.pqsoft.clientReport.action;
...
import com.pqsoft.clientReport.service.ClientReportTableService;
...
public class ClientReportTableAction extends Action {
	
	private final String _PATH = "clientReportTable/";
	// **实现类服务
	ClientReportTableService service = new ClientReportTableService();

	//页面自定义显示列的下拉列表所需，同时也是sql所需
	private String[] titlesName = new String[] { "省份", "城市", "客户名称", "证件号码",
			"客户状态", "客户等级", "联系方式", "客户经理", "创建时间", "户籍/家庭地址", "注册/办公地址" };
	private String[] sqlsName = new String[] { "PROVINCE_NAME", "CITY_NAME",
			"CUST_NAME", "CUST_CODE", "CUST_STATUS", "CUST_LEVER", "CUST_PHONE", "CLERK_NAME", "CREATE_TIME",
			"NPCOMP", "LPCOMP" };
	...
	@aPermission(name = { "报表管理", "客户报表", "客户统计" })
	...
	public Reply execute() {
		...
		List<Object> REPORT_TYPE_list = (List<Object>) new DataDictionaryMemcached().get("报表-客户-统计类型");
		context.put("REPORT_TYPE_list", REPORT_TYPE_list);
		context.put("chartReport", utilChart.to_chartPie( _getParameters(), "报表-客户-统计类型", "客户统计", "clientReport.showGroupClientType","1"));
		return new ReplyHtml(VM.html(_PATH+"graphTJClientChart.vm", context));
	}
	
	@aAuth(type = aAuth.aAuthType.USER)
	@aPermission(name = { "报表管理", "客户报表", "客户分析" })
	@aDev(code = "170053", email = "jinfu@pqsoft.cn", name = "King")
	public Reply toChartFXClient() {
		...
		context.put("REPORT_DATE_list", REPORT_DATE_list);
		List<Object> REPORT_TYPE_list = (List<Object>) new DataDictionaryMemcached().get("报表-客户-分析-统计类型");
		context.put("REPORT_TYPE_list", REPORT_TYPE_list);
		...
		String chart=new UtilChart().to_chartLine(json, "报表-客户-分析-统计类型", "客户分析", "clientReport.REPORT_X_MONTH,clientReport.CUST_REPORT_TYPE,clientReport.showGroupClientFX","1");
		context.put("chartReport", chart);
		return new ReplyHtml(VM.html(_PATH+"graphFXClientChart.vm", context));
	}
	...
	@aDev(code = "170053", email = "jinfu@pqsoft.cn", name = "King")
//	@aPermission(name = { "数据统计", "客户报表", "客户统计" })
	@SuppressWarnings("unchecked")
	public Reply toChartNewClient() {
		VelocityContext context = new VelocityContext();
		List<Object> REPORT_DATE_list = (List<Object>) new DataDictionaryMemcached().get("报表-客户统计-时间-区间");
		context.put("REPORT_DATE_list", REPORT_DATE_list);
		List<Object> REPORT_TYPE_list = (List<Object>) new DataDictionaryMemcached().get("报表-客户-统计类型");
...
//		context.put("chartReport",utilChart.to_chart(param, "报表-客户-统计类型", "客户统计", "clientReport.showGroupClientType","1"));
		...
		return new ReplyHtml(VM.html(_PATH + "graphTJClient.vm", context));
	}
	...
	@aAuth(type = aAuthType.LOGIN)
	@aDev(code = "170053", email = "jinfu@pqsoft.cn", name = "King")
//	@aPermission(name = { "数据统计", "客户报表", "客户分析" })
	@SuppressWarnings("unchecked")
	public Reply toChartFXClientData() {
		Map<String, Object> param = _getParameters();
		VelocityContext context = new VelocityContext();
		List<Object> REPORT_DATE_list = (List<Object>) new DataDictionaryMemcached().get("报表-客户分析-时间-区间");
		context.put("REPORT_DATE_list", REPORT_DATE_list);
		...
		json.put("REPORT_DATE", "4");//默认为本年
		json.put("REPORT_TYPE", "1");//默认为新增客户
		context.put("chartReport",new UtilChart().to_chartLine(json, "报表-客户-分析-统计类型", "客户分析", "clientReport.REPORT_X_MONTH,clientReport.CUST_REPORT_TYPE,clientReport.showGroupClientFX","1"));
		/********************************控制自定义显示列********************/
		context.put("columnInit", utilChart.parseColumnInit("客户统计", titlesName, sqlsName));
		/********************************************************************/
		return new ReplyHtml(VM.html(_PATH + "graphFXClient.vm", context));
	}
...
//	@aPermission(name = { "数据统计", "超级表", "列表导出" })
	...
	public ReplyAjax exportExcel(){
		...
		new POIExcelUtil().expExcel_2007(m,"客户统计"+"_"+DateUtil.getSysDate()+"_"+Math.abs(new Random(100).nextInt())+".xlsx", "客户统计", titleName,sql_columns, service.clientList(m));
